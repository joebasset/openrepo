...
ASSET_DIR="$ROOT_DIR/internal/templates/assets/ionic-react"
mkdir -p "$ROOT_DIR/.tmp"
WORK_DIR="$(mktemp -d "$ROOT_DIR/.tmp/templates-ionic-react-XXXXXX")"
APP_NAME="app"
...
(
  cd "$WORK_DIR"
  CI=1 pnpm create vite@latest "$APP_NAME" --template react-ts
)
...
printf 'refreshed %s from vite react-ts plus ionic packages\n' "$ASSET_DIR"
